    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as resp:
            if resp.status != 200:
                ...
                return
            data = resp.read()
        # write bytes to dest (outer try will catch errors)
...
if __name__ == '__main__':
    ...
    try:
        # Fetch cookies from configured URL (if provided), then log cookie env var status for debugging in Render logs
        try:
